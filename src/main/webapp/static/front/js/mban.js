...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = $('#actor3 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actor3').width(w * n);
	$('#actor3 li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlay3 .mc').width(numWidth);
	$('#imgPlay3 .num').css('left', _left);
	...
	$('.make_ban #imgPlay3 .next_m').click(function() {
		slide(1)
	});
	$('.make_ban #imgPlay3 .prev_m').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actor3').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actor3').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actor3').css('marginLeft') != -i * w + 'px') {
			$('#actor3').css('marginLeft', -i * w + 'px');
			$('#actor3').fadeOut(0,
			function() {
				$('#actor3').fadeIn(500)
			})
