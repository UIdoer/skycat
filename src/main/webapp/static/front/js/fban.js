...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = $('#actor1 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actor1').width(w * n);
	$('#actor1 li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlay1 .mc').width(numWidth);
	$('#imgPlay1 .num').css('left', _left);
	...
	$('.fre_ban #imgPlay1 .nextf').click(function() {
		slide(1)
	});
	$('.fre_ban #imgPlay1 .prevf').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actor1').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actor1').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actor1').css('marginLeft') != -i * w + 'px') {
			$('#actor1').css('marginLeft', -i * w + 'px');
			$('#actor1').fadeOut(0,
			function() {
				$('#actor1').fadeIn(500)
			})
