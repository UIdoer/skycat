...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = $('#actor6 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actor6').width(w * n);
	$('#actor6 li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlay6 .mc').width(numWidth);
	$('#imgPlay6 .num').css('left', _left);
	...
	$('.tel_ban #imgPlay6 .next_t').click(function() {
		slide(1)
	});
	$('.tel_ban #imgPlay6 .prev_t').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actor6').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actor6').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actor6').css('marginLeft') != -i * w + 'px') {
			$('#actor6').css('marginLeft', -i * w + 'px');
			$('#actor6').fadeOut(0,
			function() {
				$('#actor6').fadeIn(500)
			})
