...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = $('#actor5 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actor5').width(w * n);
	$('#actor5 li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlay5 .mc').width(numWidth);
	$('#imgPlay5 .num').css('left', _left);
	...
	$('.home_ban #imgPlay5 .next_h').click(function() {
		slide(1)
	});
	$('.home_ban #imgPlay5 .prev_h').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actor5').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actor5').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actor5').css('marginLeft') != -i * w + 'px') {
			$('#actor5').css('marginLeft', -i * w + 'px');
			$('#actor5').fadeOut(0,
			function() {
				$('#actor5').fadeIn(500)
			})
