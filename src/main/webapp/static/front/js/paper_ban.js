...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 228;
	var n = $('#actorpa li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actorpa').width(w * n);
	$('#actorpa li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlaypa .mc').width(numWidth);
	$('#imgPlaypa .num').css('left', _left);
	...
	$('.paper_ban #imgPlaypa .nextpa').click(function() {
		slide(1)
	});
	$('.paper_ban #imgPlaypa .prevpa').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actorpa').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actorpa').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actorpa').css('marginLeft') != -i * w + 'px') {
			$('#actorpa').css('marginLeft', -i * w + 'px');
			$('#actorpa').fadeOut(0,
			function() {
				$('#actorpa').fadeIn(500)
			})
