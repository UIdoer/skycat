...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 228;
	var n = $('#actorba li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actorba').width(w * n);
	$('#actorba li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlayba .mc').width(numWidth);
	$('#imgPlayba .num').css('left', _left);
	...
	$('.bb_ban #imgPlayba .nextba').click(function() {
		slide(1)
	});
	$('.bb_ban #imgPlayba .prevba').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actorba').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actorba').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actorba').css('marginLeft') != -i * w + 'px') {
			$('#actorba').css('marginLeft', -i * w + 'px');
			$('#actorba').fadeOut(0,
			function() {
				$('#actorba').fadeIn(500)
			})
