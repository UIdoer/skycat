...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 228;
	var n = $('#actorm li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actorm').width(w * n);
	$('#actorm li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlaym .mc').width(numWidth);
	$('#imgPlaym .num').css('left', _left);
	...
	$('.milk_ban #imgPlaym .nextm').click(function() {
		slide(1)
	});
	$('.milk_ban #imgPlaym .prevm').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actorm').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actorm').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actorm').css('marginLeft') != -i * w + 'px') {
			$('#actorm').css('marginLeft', -i * w + 'px');
			$('#actorm').fadeOut(0,
			function() {
				$('#actorm').fadeIn(500)
			})
