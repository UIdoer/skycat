...
$(document).ready(function() {
	...
	var speed = 500;
	var w = 211;
	var n = $('#actor4 li').length;
	var numWidth = n * 18;
	var _left = (w - (numWidth + 26)) / 2;
	var c = 0;
	$('#actor4').width(w * n);
	$('#actor4 li').each(function(i) {
		str += '<span></span>'
	});
	$('#numInner').width(numWidth).html(str);
	$('#imgPlay4 .mc').width(numWidth);
	$('#imgPlay4 .num').css('left', _left);
	...
	$('.baby_ban #imgPlay4 .next_b').click(function() {
		slide(1)
	});
	$('.baby_ban #imgPlay4 .prev_b').click(function() {
		slide( - 1)
	});
	function slide(j) {
		if ($('#actor4').is(':animated') == false) {
			c += j;
			if (c != -1 && c != n) {
				$('#actor4').animate({
					'marginLeft': -c * w + 'px'
	...
	function fade(i) {
		if ($('#actor4').css('marginLeft') != -i * w + 'px') {
			$('#actor4').css('marginLeft', -i * w + 'px');
			$('#actor4').fadeOut(0,
			function() {
				$('#actor4').fadeIn(500)
			})
