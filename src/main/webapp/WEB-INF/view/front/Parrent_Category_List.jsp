...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    ...
    <script type="text/javascript" src="${sourcePath}/js/parent-categoryList-sort-query.js"></script>
    <script type="text/javascript">
    function findPage(pageNum){
    	location = '${basePath}/good/findGoodByParrentCategoryId?parentCategoryId=${parentCategoryId}&condition=${condition}&sort=${sort}&pageNum='+pageNum;
    }
    </script>
        ...
        <div class="l_list">
        	...
        	<div class="list_t">
            	<span class="fl list_or">
                	<a id="defualt-key"   title="默认排序" <c:if test="${condition eq 'defualt-key'}">class='now'</c:if>  onclick="defaultSort('${basePath}/','${parentCategoryId}')">默认</a>
                	<c:choose >
                		<c:when test="${condition eq 'price-key' && sort eq 'desc'}">
                			 <a href="#" id="price-key" data-sort="asc" title="价格从低到高"  onclick="priceSort('${basePath}/','${parentCategoryId}')" <c:if test="${condition eq 'price-key'}">class='now'</c:if> >价格</a>
                		</c:when>
                		<c:otherwise>
                			<a href="#" id="price-key" data-sort="desc" title="价格从高到低"  onclick="priceSort('${basePath}/','${parentCategoryId}')" <c:if test="${condition eq 'price-key'}">class='now'</c:if> >价格</a>
                		</c:otherwise>
                	...
                	<c:choose>
                		<c:when test="${condition eq 'newGood-key' && sort eq 'desc'}">
                			<a href="#" id="newGood-key" data-sort="asc" title="按最远上架时间"  onclick="newGood('${basePath}/','${parentCategoryId}')" <c:if test="${condition eq 'newGood-key'}">class='now'</c:if> >新品</a>
                		</c:when>
                		<c:otherwise>
                			<a href="#" id="newGood-key" data-sort="desc" title="按最近上架时间"  onclick="newGood('${basePath}/','${parentCategoryId}')" <c:if test="${condition eq 'newGood-key'}">class='now'</c:if> >新品</a>
                		</c:otherwise>
