...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    ...
    <script type="text/javascript" src="${sourcePath}/js/allGood-sort.js"></script>
    <script type="text/javascript">
    function findPage(pageNum){
    	location = '${basePath}/good/findAllGood?condition=${condition}&sort=${sort}&pageNum='+pageNum;
    }
    </script>
            	...
            	<span class="fl list_or">
                	...
                	<c:choose >
                		<c:when test="${condition eq 'price-key' && sort eq 'desc'}">
                			 <a href="#" id="price-key" data-sort="asc" title="价格从低到高"  onclick="priceSort('${basePath}/')" <c:if test="${condition eq 'price-key'}">class='now'</c:if> >价格</a>
                		</c:when>
                		<c:otherwise>
                			<a href="#" id="price-key" data-sort="desc" title="价格从高到低"  onclick="priceSort('${basePath}/')" <c:if test="${condition eq 'price-key'}">class='now'</c:if> >价格</a>
                		</c:otherwise>
                	...
                	<c:choose>
                		<c:when test="${condition eq 'newGood-key' && sort eq 'desc'}">
                			<a href="#" id="newGood-key" data-sort="asc" title="按最远上架时间"  onclick="newGood('${basePath}/')" <c:if test="${condition eq 'newGood-key'}">class='now'</c:if> >新品</a>
                		</c:when>
                		<c:otherwise>
                			<a href="#" id="newGood-key" data-sort="desc" title="按最近上架时间"  onclick="newGood('${basePath}/')" <c:if test="${condition eq 'newGood-key'}">class='now'</c:if> >新品</a>
                		</c:otherwise>
